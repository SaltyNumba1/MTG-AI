...
- **NeMo model prompt alignment** — the system prompt and user-message format are now aligned to the NeMo 12B model's fine-tuning format (plain deck-list output: `1 Card Name` per line, with a direct action directive at the end of the user message). The previous `card_indices` JSON format was never part of the model's training data, causing it to respond by describing the prompt instead of building a deck. The parser in `extract_json` is extended to handle this native output format.
- **Pro model context window expanded** — `LLAMA_CTX` increased from 20,480 → 32,768 tokens, matching the NeMo model's trained `num_ctx`. Large-collection deck builds were being truncated mid-output (prompt ~19,963 tokens leaving only ~517 for generation), causing 400 errors. The larger KV cache (~5.1 GB vs ~3.2 GB) is well within the 16 GB VRAM of supported hardware.
- **Generation temperature reduced** — LLM sampling temperature lowered from 0.70–0.88 to 0.45–0.65 for more consistent, instruction-following output.
...
- **Robust JSON / deck-list parser** — `extract_json` handles the `{"description":"Card Name","quantity":1}` hallucination format, the native `1 Card Name` deck-list format, and returns a `_parse_failed` sentinel as a last resort so deck generation always completes gracefully.
